...
async function upload() {
  ...
  if (previewCanvas.value) {
    ...
    const blob = await new Promise<Blob | null>((resolve) => {
      selectedCanvas.toBlob(resolve, "image/png");
    });
  ...
  if (res.result) {
    ...
    document.getElementById(
      "imguri"
    ).innerHTML = `<img src="${result.translation_mask}" style="position: absolute; top: ${y}px; left: ${x}px;"/>`;
    socket.close();
  }
...
<template>
  <div class="flex-1 flex justify-between items-center gap-6">
    <div class="flex-1 flex flex-col min-w-0 h-full">
      ...
      >
        <div id="imguri"></div>
        <canvas ref="previewCanvas" class="w-full h-auto"></canvas>
        <div
          ref="selectionBox"
      ...
      >
        ...
        <div class="grid place-items-center w-full h-full rounded-2xl text-zinc-800">
          <div v-if="resultUri" class="flex flex-col sm:flex-row items-center gap-8">
            <!-- <img
              :src="resultUri"
